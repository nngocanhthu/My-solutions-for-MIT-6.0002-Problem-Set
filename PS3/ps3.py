...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        x_tile = math.floor(pos.get_x())
        y_tile = math.floor(pos.get_y())
        if not ((x_tile, y_tile) in self.tiles.keys()):
            raise ValueError
        else:
            self.tiles[(x_tile, y_tile)] -= capacity
            if self.tiles[(x_tile, y_tile)] < 0:
                self.tiles[(x_tile, y_tile)] = 0
    ...
    def is_position_in_room(self, pos):
        ...
        x_tile = math.floor(pos.get_x())
        y_tile = math.floor(pos.get_y())
        if (x_tile, y_tile) in self.tiles.keys():
            return True
        return False
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        x_tile = math.floor(pos.get_x())
        y_tile = math.floor(pos.get_y())
        if (x_tile, y_tile) in self.tiles.keys() and not self.is_tile_furnished(x_tile, y_tile):
            return True
        return False
